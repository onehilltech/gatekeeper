...
function Oauth2Controller (models) {
  ...
  function initializeExchanges () {
    ...
    self._server.exchange ('password', oauth2orize.exchange.password (function (client, username, password, scope, body, done) {
      ...
      self._clientModel.findById (clientId, function (err, client) {
        ...
        if (!client.enabled)
          return done (new oauth2orize.AuthorizationError('client is not enabled', 'unauthorized_client'));
        ...
        self._accountModel.authenticate(username, password, function (err, account) {
          ...
          self._accessTokenModel.newUserToken(TOKEN_LENGTH, client.id, account.id, function (err, accessToken) {
            if (err)
              return done (err);
